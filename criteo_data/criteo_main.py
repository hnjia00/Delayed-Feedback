...
if __name__ == '__main__':
    ...
    # config.B = 3000
    config.B = 12000
    config.N = int(config.criteo_allsize / config.B)
    # config.N = 2
    mu = 0.8
    w = 1.0

    # f = open('./result/final_result/all/merit/CVR_CTCVR.txt', 'a+')
    for i in range(1):
        # D = D_init
        # _D = _D_init
        ...
        # modify = "no"
        # mode = 'DFM'
        # reward_dfm, c, v = Algo_main(D, config.B, config.N, _D, _algo, mode, modify=modify)
        # print(c)
        # print(v)
        # print(c, file=f)
        # print(v, file=f)
        # f = open('result/test/replay_continuous_coef_reward_DFM_B' + str(config.B), 'w')
        # print(reward_dfm, file=f)
        ...
        modify = ''
        mode = 'UCB'
        reward, c, v = Algo_main(np.array(D), config.B, config.N, _D, _algo, mode, modify=modify, xi=0.9)
        print(reward)
        print(np.sum(np.array(reward)), v[-1]/c[-1], v[-1]/(config.N * config.B))

        # print(c)
        # print(v)
        # f = open('result/test/UCB_all_reward_B' + str(config.B) + modify, 'w')
        # print(reward, file=f)
        ...
        # _algo.miu_ucb = 1.1
        # modify = 'no'
        # reward1, c, v = Algo_main(np.array(D), config.B, config.N, _D, _algo, mode, modify=modify)
        # print(c)
        # print(v)

        # print(np.array(reward)-np.array(reward1))
        # f = open('result/test/UCB_all_reward_B' + str(config.B) + modify, 'w')
