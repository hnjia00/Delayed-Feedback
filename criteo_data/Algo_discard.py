...
def Algo_main(D_n, B, N, _D, _algo=None, mode='no', modify=''):
    ...
    for n in tqdm(range(N)):
        ...
        for i in range(B):
            ...
            if not(c == 1 and y == 0):
                D_train.append(D_temp[i])

        REWARD.append(r_sum)

        # 存储线上数据
        D_temp = np.array(D_train)
        # 经验池还没填满
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        # _algo.continuous = 1
        # _, c, v = Algo_main(D_init, config.B, config.N, _D_init, _algo, mode, modify=modify)
        # print(c)
        # print(v)
        ...
        # _algo.miu_ucb = mu
        # modify = ''
        # reward, c, v = Algo_main(np.array(D), config.B, config.N, _D, _algo, mode, modify=modify)
        # print(reward)
        ...
        print(reward1)
        print(np.sum(np.array(reward1)), v[-1] / c[-1], v[-1] / (config.N * config.B))
        # print(c)
        # print(v)
        ...
        R.append(np.sum(np.array(reward1)))
        CVR.append(v[-1]/c[-1])
